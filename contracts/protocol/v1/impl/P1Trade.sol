    ...
    P1Settlement
{
        ...
        pure
    {
        for (uint256 i = 0; i < accounts.length; i++) {
            ...
            // Note: If the account is undercollateralized and position is zero, then margin is
            // negative and the account has no positive value.
            Require.that(
                finalBalance.positionIsPositive == initialBalance.positionIsPositive,
                "account is undercollateralized and position is zero or changed signs",
                account
            );
            Require.that(
